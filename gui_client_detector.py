#!/usr/bin/env python3
# gui_client_detector.py (responsive PanedWindow version)
...
class EnhancedGuiClientDetectorApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Label(toolbar, text="Interface:", style='Header.TLabel').pack(side='left')
        self.iface_var = tk.StringVar()
        self.iface_entry = ttk.Entry(toolbar, width=12, textvariable=self.iface_var)
        self.iface_entry.pack(side='left', padx=(6,8))
        ttk.Button(toolbar, text="Discover", command=self.discover_interfaces).pack(side='left', padx=4)
        ttk.Button(toolbar, text="Start Monitor", command=self.on_start_monitor).pack(side='left', padx=4)
        ttk.Button(toolbar, text="Stop Monitor", command=self.on_stop_monitor).pack(side='left', padx=4)
        ttk.Button(toolbar, text="Scan APs", command=self.on_scan_aps).pack(side='left', padx=4)

        # Status on right
        status_frame = ttk.Frame(toolbar)
        status_frame.pack(side='right', anchor='e')
        self.status_var = tk.StringVar(value="Idle")
        self.progress = ttk.Progressbar(status_frame, mode='indeterminate', style='Horizontal.TProgressbar', length=180)
        self.progress.pack(side='right', padx=(6,0))
        ttk.Label(status_frame, textvariable=self.status_var, style='Status.TLabel').pack(side='right', padx=(6,0))

        # Filters frame
        filters = ttk.Frame(self, padding=(8,6))
        filters.pack(side='top', fill='x', padx=6, pady=(4,6))
        ...
        self.search_entry.grid(row=0, column=1, padx=(6,4), sticky='ew')
    ...
    def _create_main_panes(self):
        ...
        self.tree.column('bssid', width=260, anchor='w', stretch=True)
        ...
        ysb = ttk.Scrollbar(ap_table_frame, orient='vertical', command=self.tree.yview)
        ysb.grid(row=0, column=1, sticky='ns')
        xsb = ttk.Scrollbar(ap_table_frame, orient='horizontal', command=self.tree.xview)
        ...
        self.susp_tree.column('bssid', width=260, anchor='w', stretch=True)
        self.susp_tree.column('ssid', width=220, anchor='w', stretch=True)
        ...
        ysb2 = ttk.Scrollbar(susp_frame, orient='vertical', command=self.susp_tree.yview)
            ...
            while True:
                ...
                if kind == 'add_ap':
                    ...
                    for b, val in results.items():
                        ...
                        self.susp_tree.insert('', tk.END, values=(b, ss, score, sev), tags=(sev,))
