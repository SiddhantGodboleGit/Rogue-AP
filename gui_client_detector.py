...
A lightweight modification of your gui.py that adds:
- Whitelist entries (SSIDs & BSSIDs)
- "Detect Rogue APs" button
- A "Suspicious APs" panel showing BSSID, SSID, severity and reasons.

Drop into same folder as gui.py/scanner.py/ap_manager.py and run:
    sudo python3 gui_client_detector.py

Note: detection uses client_detector.detect_rogue_aps and relies on scanner.scan_aps
to collect info. If you want RSSI-based checks, modify scanner.scan_aps to attach 'rssi'
to the info dict (packet.dBm_AntSignal if scapy has it).
"""
...
# import existing modules as before
try:
    import scanner
except Exception:
    scanner = None
...
class GuiClientDetectorApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Rogue AP — Client Detector GUI")
        ...
        try:
            ...
            pass
        # Use the same parchment-inspired palette and styles as `gui.py`
        ...
        try:
            ...
            # Make Treeview rows slightly taller so text doesn't visually overlap
            # Avoid forcing a font here; some fonts (e.g. Garamond) interact oddly with row metrics
    ...
    def _create_toolbar(self):
        # toolbar row 1: interface and primary actions
        ...
        self.iface_entry.grid(row=0, column=1, padx=(4,6))
        ...
        ttk.Label(toolbar, textvariable=self.status_var, style='Status.TLabel').grid(row=0, column=99, sticky='e')
        # make columns expand so status sits at right
        toolbar.grid_columnconfigure(98, weight=1)

        # second row: whitelist inputs and detect button (separate to avoid crowding)
        filters = ttk.Frame(self, padding=(6,2), style='Toolbar.TFrame')
        ttk.Label(filters, text="Whitelist SSIDs (comma):").grid(row=0, column=0, padx=(0,6), sticky='w')
        self.whitelist_ssid_var = tk.StringVar(value="")
        ttk.Entry(filters, width=40, textvariable=self.whitelist_ssid_var).grid(row=0, column=1, sticky='w')
        ttk.Label(filters, text="Whitelist BSSIDs (comma):").grid(row=0, column=2, padx=(12,6), sticky='w')
        self.whitelist_bssid_var = tk.StringVar(value="")
        ttk.Entry(filters, width=30, textvariable=self.whitelist_bssid_var).grid(row=0, column=3, sticky='w')
        ttk.Button(filters, text="Detect Rogue APs", command=self.on_detect).grid(row=0, column=4, padx=(12,0))
    ...
    def _create_main_panes(self):
        ...
        # set a reasonable visible height (rows) so the widget lays out predictably
        ...
        # BSSID is fixed-width-ish; SSID should expand to fill remaining space
        self.tree.column('bssid', width=240, anchor=tk.W, stretch=False)
        self.tree.column('ssid', width=520, anchor=tk.W, stretch=True)
        self.tree.column('channel', width=60, anchor='center', stretch=False)
        ...
        ctrl = ttk.Frame(left)
        ttk.Button(ctrl, text="Clear", command=self._clear_results).pack(side=tk.LEFT)
        ttk.Button(ctrl, text="Export CSV", command=self.export_csv).pack(side=tk.LEFT, padx=(6,0))
        ctrl.pack(anchor=tk.W, pady=(6,0))

        right = ttk.Frame(main, padding=8)
        ttk.Label(right, text="Suspicious APs", style='Header.TLabel').pack(anchor=tk.W)
        scol = ("bssid","ssid","severity")
        susp_wrap = ttk.Frame(right, style='Panel.TFrame')
        self.susp_tree = ttk.Treeview(susp_wrap, columns=scol, show='headings', height=8)
        ...
        for c in scol:
            self.susp_tree.heading(c, text=c.upper())
        self.susp_tree.column('bssid', width=260, anchor=tk.W, stretch=False)
        self.susp_tree.column('ssid', width=300, anchor=tk.W, stretch=True)
        ...
        # Reasons box
        ttk.Label(right, text="Reasons / Details").pack(anchor=tk.W, pady=(8,0))
        self.reasons_text = tk.Text(right, height=16, wrap='word')
        self.reasons_text.configure(state='disabled', background=self._bg_panel)
    ...
    def on_start_monitor(self):
        ...
        if scanner is None:
            ...
            return
        # normalize to base iface (strip trailing 'mon' if present)
    ...
    def _scan_aps_thread(self, mon_iface, timeout):
        try:
            ...
            def on_ap(bssid, info):
                ...
                self._aps_info[bssid.lower()] = info if isinstance(info, dict) else {'ssid': ssid}
                self._queue.put(('add_ap', (bssid, ssid, ch)))
            aps = scanner.scan_aps(mon_iface, timeout=timeout, on_ap=on_ap)
            self._append_log(f"Scan complete: {len(aps)} AP(s) returned by scanner")
        except Exception as e:
            ...
            while True:
                ...
                if kind == 'add_ap':
                    ...
                    for b, val in results.items():
                        ...
                        self.susp_tree.insert('', tk.END, values=(b, ss, sev))
                elif kind == 'log':
    ...
    def _on_ap_selected(self, event):
        try:
            ...
            bssid = vals[0]
            info = self._aps_info.get(bssid.lower(), {})
            self.reasons_text.configure(state='normal')
            self.reasons_text.delete(1.0,'end')
            self.reasons_text.insert('end', f"Details for {bssid}:\n\n")
    ...
    def _on_susp_selected(self, event):
        try:
            ...
            self.reasons_text.configure(state='normal')
            self.reasons_text.delete(1.0,'end')
            self.reasons_text.insert('end', f"Suspicious AP {bssid} ({ssid})\nSeverity: {sev}\n\nReasons:\n")
            if rec:
